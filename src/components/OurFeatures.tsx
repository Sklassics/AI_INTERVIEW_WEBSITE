...
const AdvancedFeaturesSection: React.FC<AdvancedFeaturesSectionProps> = ({
  ...
  // Scroll shuffle/entrance with snap on desktop
  useEffect(() => {
    if (typeof window === "undefined" || window.innerWidth < 1024) return;
    const CARD_SCROLL_DIST = 440; // You can adjust this value or use window.innerHeight for full screen
    ...
    const ctx = gsap.context(() => {
      ...
      // animate transitions synced to scroll segments with snapping
      cardRefs.current.forEach((card, i) => {
        if (i > 0 && card) {
          tl.to(
            card,
    ...
    <section className="relative w-full overflow-hidden bg-gradient-to-br from-background via-background to-accent/5 text-white">
      ...
      <section className="hidden lg:block w-full px-0 pt-16 relative z-10">
        <div className="mx-auto max-w-none font-bruno font-bold">
          ...
          <div
            ref={headerRef}
            className="sticky top-0 left-0 right-0 z-20 backdrop-blur-lg bg-gradient-to-br from-background via-background to-accent/5 rounded-xl shadow-lg py-6 transition-transform"
          >
